------------------------------------------------------------------------------
--
-- alg_smp.as: A basic constructor for Recursive Multivariate Polynomials
--
--------------------------------------------------------------------------------
...
-- dom SparseMultivariatePolynomial
...
+++ `SparseMultivariatePolynomial(R,V)' provides a basic domain for 
+++ multivariate polynomials with coefficients in `R' and variables 
+++ in `V'. The representation is sparse and recursive by using the
+++ `SparseUnivariatePolynomial' constructor.
+++ Author: Marc Moreno Maza
+++ Date Created: 02/1996
+++ Date Last Update: 12/07/2001

SparseMultivariatePolynomial(_
  R :Ring, _
  V : VariableType):  RecursiveMultivariatePolynomialCategory0(R,V) with {
	...
	-- s is sorted lowest first, so by cons'ing in that order, we reverse
        lv: List(V) := empty;
